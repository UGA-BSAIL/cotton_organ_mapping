...
% 3) "make_tree_connectedâ€?: Removed "Trunk(Base) = false;" at the beginning
%     of the function as unnecessary and to prevent errors in a special case
%     where the Trunk is equal to Base.
%	4) "make_tree_connectedâ€?: Removed from the end the generation of "Trunk"
%     again and the new call for the function
%	5) "make_tree_connectedâ€?: Increased the minimum distance of a component
...
elseif inputs.OnlyTree
  ...
  I = Ce(Trunk0,3) > HBottom+min(0.003*aux.Height,0.45);
  Trunk = Trunk0(I);
  Trunk = union(Trunk,vertcat(Nei{Trunk}));
  Trunk = union(Trunk,TrunkBot);

  BaseHeight = min(1.5,0.002*aux.Height);
  % Determine the base
  Bot = min(Ce(Trunk,3));
  ...
  Bot = min(Ce(Trunk,3));
  Ground = Ce(:,3) < Bot+0.001*aux.Height;
  Forb(Ground) = true;
  Forb(Trunk) = false;
  cover.ball = Bal;
end
